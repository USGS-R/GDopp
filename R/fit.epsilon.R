...
fit.epsilon <- function(chunk.adv,freq=32, lower= 20,upper=80){
  ...
  f <- function (x, w.s, k){
    ...
    eps.2.3 <- x^(2/3)
    epsilon.rmse <- rmse(log((0.52*eps.2.3*k)*k^(-5/3)),log(w.s))
    
    return(epsilon.rmse)
  }
  epsilon <- optimize(f, c(10^(-10), 1), tol = 0.0001, w.s = wavenum.spectra[use.i], k = wavenum[use.i])$minimum
